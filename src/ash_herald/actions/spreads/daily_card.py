...
from ash_herald import texts
from datetime import datetime, date

from ash_herald.utils.keyboard import get_main_keyboard
...
async def daily_card(bot, call, session):
    ...
    deck = session.deck 
    logger.info(f"User: {session.name}, using deck: {deck}")

    loading_msg1 = await bot.send_message(
        chat_id, 
        "🕯 <i>Пламя свечи танцует в полумраке...</i>", 
        parse_mode="HTML")
    time.sleep(1.5)

    loading_msg2 = await bot.send_message(
        chat_id, 
        "ⴲ <i>Взгляд скользит по древним символам...</i>", 
        parse_mode="HTML")
